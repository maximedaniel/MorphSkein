...
double linearAcceleration(float currentTime, float startVel, float endVel, float duration) {
  return  startVel + (endVel - startVel)  * min(1, currentTime/duration);
}
double trapezoidalVelocity(float currentTime, float startPos, float endPos, float duration) {
  ...
  if (currentTime < accelerationTime) {
    return  max_velocity * min(1, currentTime/accelerationTime);
  } 
  else if (currentTime < accelerationTime + constantVelocityTime) {
    return  max_velocity;
  } 
  else if (currentTime < duration) {
    return max_velocity * max(0, (duration - currentTime)/decelerationTime);
    } 
  else {
    return 0;
  }
...
double trapezoidalPosition(float currentTime, float startPos, float endPos, float duration) {
  ...
  if (currentTime < accelerationTime) {
    return startPos + totalDistanceAcceleration * min(1, currentTime/accelerationTime);
  } 
  else if (currentTime < accelerationTime + constantVelocityTime) {
    return startPos + totalDistanceAcceleration + totalDistanceConstant * min(1, (currentTime - accelerationTime)/constantVelocityTime);
  } 
  else if (currentTime < duration) {
    return startPos + totalDistanceAcceleration + totalDistanceConstant + totalDistanceDeceleration * min(1, (currentTime - (accelerationTime + constantVelocityTime))/decelerationTime);
    } 
  else {
    return endPos;
  }
          ...
          _curr_position = curr_value; 
           double _pid_correction_velocity = 0;
            ...
            if (MotionPlanner::isReaching()){
              ...
              _target_velocity = _start_velocity + (_end_velocity - _start_velocity)  * min(1, _curr_duration/_target_duration);
