...
*/
My_Adafruit_DotStar::My_Adafruit_DotStar(uint16_t n, uint8_t o, SPIClass *spi,  uint32_t freq)
    : numLEDs(n), brightness(0), pixels(NULL), rOffset(o & 3),
      gOffset((o >> 2) & 3), bOffset((o >> 4) & 3), 
      buffer (NULL), nb_start_frame_bytes (0), nb_pixel_data_bytes (0), nb_end_frame_bytes (0), total_bytes (0) {
  spi_dev = new My_Adafruit_SPIDevice(-1, freq, SPI_BITORDER_MSBFIRST,
                                   SPI_MODE0, spi);
  updateLength(n);
}
...
/*!
  ...
  @param   o      Pixel type -- one of the DOTSTAR_* constants defined in
                  My_Adafruit_DotStar.h, for example DOTSTAR_BRG for DotStars
                  expecting color bytes expressed in blue, red, green order
                  per pixel. Default if unspecified is DOTSTAR_BRG.
  @return  Adafruit_DotStar object. Call the begin() function before use.
*/
My_Adafruit_DotStar::My_Adafruit_DotStar(uint16_t n, uint8_t data, uint8_t clock,
                                   uint8_t o)
    : brightness(0), pixels(NULL), rOffset(o & 3), gOffset((o >> 2) & 3), bOffset((o >> 4) & 3),
      buffer (NULL), nb_start_frame_bytes (0), nb_pixel_data_bytes (0), nb_end_frame_bytes (0), total_bytes (0) {
  spi_dev = new My_Adafruit_SPIDevice(-1, clock, -1, data, 8000000);
  updateLength(n);
}

/*!
  @brief   Deallocate My_Adafruit_DotStar object, set data and clock pins
           back to INPUT.
*/
My_Adafruit_DotStar::~My_Adafruit_DotStar(void) {
  free(pixels);
  free(buffer);
  if (spi_dev)
    delete (spi_dev);
...
/*!
  @brief   Initialize My_Adafruit_DotStar object -- sets data and clock pins
           to outputs and initializes hardware SPI if necessary.
*/
void My_Adafruit_DotStar::begin(void) { spi_dev->begin(); }
...
/*!
  @brief   Switch over to hardware SPI. DotStars must be connected to
           ...
           continue to be used.
*/
void My_Adafruit_DotStar::updatePins(void) {
  if (spi_dev)
    delete (spi_dev);
  spi_dev = new My_Adafruit_SPIDevice(-1, 8000000);
  spi_dev->begin();
}
...
/*!
  ...
  @param   clock  Arduino pin number for clock out.
*/
void My_Adafruit_DotStar::updatePins(uint8_t data, uint8_t clock) {
  if (spi_dev)
    delete (spi_dev);
  spi_dev = new My_Adafruit_SPIDevice(-1, clock, -1, data, 8000000);
  spi_dev->begin();
...
/*!
  @brief   Change the length of a previously-declared My_Adafruit_DotStar
  ...
  @note    This function is deprecated, here only for old projects that
           ...
           'new' keyword.
*/
void My_Adafruit_DotStar::updateLength(uint16_t n) {
  free(pixels);
  free(buffer);
  ...
  if ((pixels = (uint8_t *)malloc(bytes))) {
    numLEDs = n;
    clear();
    nb_start_frame_bytes = 4;
    nb_pixel_data_bytes = numLEDs * 4;
    nb_end_frame_bytes = (numLEDs + 15) / 16;
    total_bytes = nb_start_frame_bytes + nb_pixel_data_bytes + nb_end_frame_bytes;
    buffer = (uint8_t *)malloc(total_bytes);
    // start frame
    uint16_t j = 0;
    while (j < nb_start_frame_bytes){
      buffer[j++] = 0x00;
    }
    j = nb_start_frame_bytes + nb_pixel_data_bytes;
    while (j < total_bytes){
      buffer[j++] = 0xFF;
    }
...
unsigned long getNanos(unsigned* prev_ccount, unsigned long* curr_nanos, uint32_t CPU_tick_ns,  unsigned long modulo_nanos){
        ...
        *curr_nanos = (*curr_nanos + delta_ccount * CPU_tick_ns) % modulo_nanos;
...
*/
void My_Adafruit_DotStar::show(void) {
  if (!pixels)
    return;
  ...
  unsigned long next_nanos = 0;
  unsigned prev_ccount = 0;
  uint32_t CPU_tick_ns = 4;
  unsigned long modulo_nanos = 30000000;

  // fill buffer
  uint16_t j = nb_start_frame_bytes;
  while (j < nb_start_frame_bytes + nb_pixel_data_bytes){
    buffer[j++] = 0xFF; // START
    buffer[j++] = *ptr++; // R 
    buffer[j++] = *ptr++; // G
    buffer[j++] = *ptr++; // B
...
//   for (uint16_t j = 0; j < nb_start_frame_bytes; j++) start_frame_bytes [j] = 0x00;
//   for (uint16_t j = 0; j < nb_end_frame_bytes; j++) end_frame_bytes [j] = 0xFF;
//   Serial.print("mine: ");
//   for (uint16_t j = 0; j < nb_pixel_data_bytes; j+=4) {
//       pixel_data_bytes[j] = 0xFF;
//       pixel_data_bytes[j+1] = *ptr++;
//       Serial.print(pixel_data_bytes[j+1]);
//       Serial.print(" ");
//       pixel_data_bytes[j+2] = *ptr++;
//       Serial.print(pixel_data_bytes[j+2]);
//       Serial.print(" ");
//       pixel_data_bytes[j+3] = *ptr++;
//       Serial.print(pixel_data_bytes[j+3]);
//       Serial.print(" ");
//  }
 //Serial.println();
  // for (uint8_t j = 0; j < nb_pixel_data_bytes; j++) {
  //     pixel_data_bytes[j++] = 0xFF;
  //     pixel_data_bytes[j++] = *ptr++;
  //     pixel_data_bytes[j++] = *ptr++;
  //     pixel_data_bytes[j] = *ptr++;
  //   //   spi_dev->transfer(0xFF); //  Pixel start
  //   //   for (i = 0; i < 3; i++)
  //   //     spi_dev->transfer(*ptr++); // R,G,B
  //   // end_frame_bytes [j] = 0xFF;
  // }
  ...
  //prev_nanos = getNanos(&prev_ccount, &curr_nanos, CPU_tick_ns, modulo_nanos);
  spi_dev->beginTransaction();
  //next_nanos =  getNanos(&prev_ccount, &curr_nanos, CPU_tick_ns, modulo_nanos);
  //Serial.print("begin transaction:");
  //Serial.println(next_nanos - prev_nanos);
   // [FULL FRAME]
   //prev_nanos = getNanos(&prev_ccount, &curr_nanos, CPU_tick_ns, modulo_nanos);
   //spi_dev->transfer(buffer, total_bytes); 
   //next_nanos = getNanos(&prev_ccount, &curr_nanos, CPU_tick_ns, modulo_nanos);
  ...
  // [START FRAME]
  //prev_nanos = getNanos(&prev_ccount, &curr_nanos, CPU_tick_ns, modulo_nanos);
  //spi_dev->transfer(start_frame_bytes, nb_start_frame_bytes); // 2344 ns
  // for (i = 0; i < 4; i++) // 5988 ns
  ...
  // // [PIXEL DATA]
  //prev_nanos = getNanos(&prev_ccount, &curr_nanos, CPU_tick_ns, modulo_nanos);
  spi_dev->transfer(buffer, total_bytes); // 2344 ns
  //spi_dev->transfer(pixel_data_bytes, nb_pixel_data_bytes); // 2344 ns
  ...
  // prev_nanos = getNanos(&prev_ccount, &curr_nanos, CPU_tick_ns, modulo_nanos);
  //spi_dev->transfer(end_frame_bytes, nb_end_frame_bytes); // 2344 ns
  // for (i = 0; i < ((numLEDs + 15) / 16); i++)
  //   spi_dev->transfer(0xFF);
  // next_nanos = getNanos(&prev_ccount, &curr_nanos, CPU_tick_ns, modulo_nanos);
  // Serial.print("end frame data:");
  // Serial.println(next_nanos - prev_nanos);
...
*/
void My_Adafruit_DotStar::clear() {
  memset(pixels, 0,
...
*/
void My_Adafruit_DotStar::setPixelColor(uint16_t n, uint8_t r, uint8_t g,
                                     uint8_t b) {
  if (n < numLEDs) {
...
/*!
  ...
  @param   c  32-bit color value. Most significant byte is 0, second is
              ...
              e.g. 0x00RRGGBB
*/
void My_Adafruit_DotStar::setPixelColor(uint16_t n, uint32_t c) {
  if (n < numLEDs) {
    uint8_t *p = &pixels[n * 3];
...
*/
void My_Adafruit_DotStar::fill(uint32_t c, uint16_t first, uint16_t count) {
  uint16_t i, end;
...
*/
uint32_t My_Adafruit_DotStar::ColorHSV(uint16_t hue, uint8_t sat, uint8_t val) {
...
/*!
  ...
  @return  'Packed' 32-bit RGB value. Most significant byte is 0, second is
           is red, then green, and least significant byte is blue.
*/
uint32_t My_Adafruit_DotStar::getPixelColor(uint16_t n) const {
  if (n >= numLEDs)
    return 0;
  uint8_t *p = &pixels[n * 3];
...
/*!
  ...
  @note    For various reasons I think brightness is better handled in
           ...
           stored.
*/
void My_Adafruit_DotStar::setBrightness(uint8_t b) {
...
*/
uint8_t My_Adafruit_DotStar::getBrightness(void) const {
...
/*!
  ...
  @return  Gamma-adjusted packed color, can then be passed in one of the
           ...
           function instead.
*/
uint32_t My_Adafruit_DotStar::gamma32(uint32_t x) {
  uint8_t *y = (uint8_t *)&x;
...
*/
void My_Adafruit_DotStar::rainbow(uint16_t first_hue, int8_t reps,
