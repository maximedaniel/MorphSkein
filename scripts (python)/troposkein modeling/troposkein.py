...
def troposkein3D(W, S, L, O, D, Pv, Ph, T, verbose=True):
    ...
    max_vertical_resolution = np.floor(Sd/Pv)
    print("max vertical resolution = %.2f pixels" %max_vertical_resolution)  if verbose else 0
    max_horizontal_resolution = int(np.floor(max_perimeter/Ph)) 
    ...
    max_vertical_resolution = px_row_count
    print("final max vertical resolution = %.2f pixels" %max_vertical_resolution)  if verbose else 0
    max_horizontal_resolution = px_col_count
    print("final max horizontal resolution = %.2f pixels" %max_horizontal_resolution)   if verbose else 0
    assert len(final_pixels_x) == px_row_count * px_col_count
